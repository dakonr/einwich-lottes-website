...
## PROJECT
Static one-page "web business card" (Web-Visitenkarte) for Einwich & Lottes GmbH, a German HVAC/plumbing trade business (Heizung/Sanitär/Solar/Wartung) near Bamberg. Pure hand-written HTML5 + one CSS file. No build step, no framework, no runtime dependencies.
...
- ZIP excludes: `.DS_Store`, `node_modules`, `.git`, `.github`, `.omo`, `tests`, `screenshots`, `playwright-report`, `test-results`, `.lighthouseci`, config files (`package.json`, `playwright.config.js`, `lighthouserc.json`, `run-tests.sh`, `.gitignore`)
- `deploy/` directory is gitignored; rebuilt fresh each CI run
- Release auto-created on `main` push with the ZIP as an asset
